...
type Params = Promise<{ storeSlug: string; "product-id": string }>
...
export default async function SeoPageRedirect({ params }: { params: Params }) {
  const { storeSlug, "product-id": productId } = await params
  redirect(`/store/${storeSlug}/products/${productId}?tab=seo`)
}
