...
export default function AttributesTab({ isSubmitting }: Props) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>السمات المخصّصة</CardTitle>
      ...
      <CardContent>
        {isPending ? (
          ...
          <div className="flex flex-col items-center gap-2 rounded-lg border-2 border-dashed bg-gray-50 py-10 text-center">
  ...
  switch (def.dataType) {
    ...
    case "BOOLEAN":
      return (
        <UiField key={defId}>
          <SysSwitch
            label={def.attributeNameAr ?? ""}
            value={current?.valueText === "true"}
            onChange={(checked) =>
              writeValue(defId, { valueText: checked ? "true" : "false" })
            }
          />
        </UiField>
      )
