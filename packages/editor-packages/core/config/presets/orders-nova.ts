...
import { createNovaThemeWalk } from "./nova-theme-walk";
import {
...
/**
 * Nova (نوفا) styling layer over the shared `/orders` presets.
 * Styling rules live in `./nova-theme-walk`; this file owns the translations
...
const themeWalk = createNovaThemeWalk(BILINGUAL);

export function createNovaOrdersPageContent(): ComponentDataOptionalId[] {
  return themeWalk(
    createOrdersPageContent(),
    "orders"
...
export function createNovaOrderDetailPageContent(): ComponentDataOptionalId[] {
  return themeWalk(
    createOrderDetailPageContent(),
    "order-detail"
  ) as ComponentDataOptionalId[];
...
export function createNovaCancelOrderZonePopup(): ComponentDataOptionalId {
  return themeWalk(
    createCancelOrderZonePopup(),
    "cancel-order"
  ) as ComponentDataOptionalId;
...
export function createNovaOrdersSitePage(): SitePage {
  return {
    path: "/orders",
    slug: "/orders",
    link: "/orders",
    ...
    iconName: "Package",
    isCustom: false,
    content: createNovaOrdersPageContent(),
  };
}

export function createNovaOrderDetailSitePage(): SitePage {
  return {
    ...
    content: createNovaOrderDetailPageContent(),
  };
}
