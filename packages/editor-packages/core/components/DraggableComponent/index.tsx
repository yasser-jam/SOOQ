...
export const DraggableComponent = ({
  ...
  const getStyle = useCallback(() => {
    ...
    const deepScrollPosition = getDeepScrollPosition(el);
    ...
    const scroll = portalContainerEl
      ? {
          x:
    ...
    const transform = accumulateTransform(el);

    if (iframe.enabled) {
      const { left, top } = getOffsetWithinBody(el);
